...
class Editor extends React.Component {
  ...
  componentDidMount() {
    ...
    state.isAdmin = role.cRankId === RANK_ADMIN;

    if (!state.isAdmin) {
      state.selectedRole = role.cRankId;
      state.selectedFunc = role.rootFuncs.map((func) => {
        return func.cId
      })
    }

    this.setState({funcDic, rankDic, ...state}, () => {
      document.getElementById("isAdmin").parentNode["Switch"].checkToggleState();
      document.getElementById("name").parentNode["Textfield"].change(role.cName);
      document.getElementById("desc").parentNode["Textfield"].change(role.cDesc);
    })
  }
                              ...
                              <div key={rank.cId} className="cell cell--4-col typography--text__center">
                                <label className="radio js-radio" htmlFor={`option-${rank.cId}`}>
                                  <input
                                    ...
                                    checked={this.state.selectedRole === rank.cId}
                                  />
